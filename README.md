...
이 프로젝트는 "일본어 UI를 보면서도 한국어로 바로 이해하고 싶다"는 생각에서 시작했습니다. 화면을 캡처하거나 별도 앱으로 넘기지 않고, 현재 보고 있는 앱 안에서 일본어 텍스트를 한국어로 바로 확인할 수 있도록 만들었습니다. **접근성 트리**에서 문자열과 좌표를 가져와, 해당 위치에 한국어 해석을 오버레이로 보여줍니다.
